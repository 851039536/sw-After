...
namespace After.DAL
{
    ...
    public partial class miscellaneous
    {
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(Model.miscellaneous model)
		{
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.miscellaneous GetModel()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select 型号,程序路径,数据路径 from miscellaneous");
			strSql.Append(" where ");
			MySqlParameter[] parameters = {
			};

			Model.miscellaneous model = new Model.miscellaneous();
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.miscellaneous DataRowToModel(DataRow row)
		{
			Model.miscellaneous model = new Model.miscellaneous();
			if (row != null)
			{
				...
				if (row["型号"] != null && row["型号"].ToString() != "")
				{
					model.型号 = row["型号"].ToString();
				}
				if (row["程序路径"] != null)
				{
					model.程序路径 = row["程序路径"].ToString();
				}
                if (row["数据路径"] != null)
                {
                    model.数据路径 = row["数据路径"].ToString();
                }
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select id,型号,程序路径,数据路径 ");
			strSql.Append(" from miscellaneous ");
			if (strWhere.Trim() != "")
			{
